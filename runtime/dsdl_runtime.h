// OpenCyphal common serialization support routines adapted for llvm-dsdl.
// This runtime is header-only and intentionally aligned with Nunavut behavior.
...
_Static_assert(sizeof(size_t) >= sizeof(size_t),
               "Unexpected target size_t width");

/// Runtime returns 0 for success and < 0 for any failure.
...
///                                                               [-- out bits --]
///
static inline size_t dsdl_runtime_saturate_fragment_bits(
...
{DSDL_RUNTIME_ASSERT(src != NULL);DSDL_RUNTIME_ASSERT(dst != NULL);DSDL_RUNTIME_ASSERT(src != dst);
    ...
    else
    {
        ...
        DSDL_RUNTIME_ASSERT(((psrc < pdst) ? ((uintptr_t)(psrc + ((src_offset_bits + length_bits + 7U) / 8U)) <= (uintptr_t)pdst) : 1));
        DSDL_RUNTIME_ASSERT(((psrc > pdst) ? ((uintptr_t)(pdst + ((dst_offset_bits + length_bits + 7U) / 8U)) <= (uintptr_t)psrc) : 1));
...
/// algorithm is employed. See @ref dsdl_runtime_copy_bits() for further details.
static inline void dsdl_runtime_get_bits(void* const output,
                                  const void* const buf,
                                  const size_t buf_size_bytes,
                                  const size_t off_bits,
                                  const size_t len_bits)
{DSDL_RUNTIME_ASSERT(output != NULL);DSDL_RUNTIME_ASSERT(buf != NULL);
...
static inline int8_t dsdl_runtime_set_bit(
    uint8_t* const buf,
    const size_t buf_size_bytes,
    const size_t off_bits,
    const bool value)
...
static inline int8_t dsdl_runtime_set_uxx(
    uint8_t* const buf,
    const size_t buf_size_bytes,
    const size_t off_bits,
...
{
    _Static_assert(64U == (sizeof(uint64_t) * 8U), "Unexpected size of uint64_t");DSDL_RUNTIME_ASSERT(buf != NULL);
...
static inline int8_t dsdl_runtime_set_ixx(
    uint8_t* const buf,
    const size_t buf_size_bytes,
    const size_t off_bits,
    const int64_t value,
...
static inline uint8_t dsdl_runtime_get_u8(const uint8_t* const buf,
                                   ...
                                   const uint8_t len_bits)
{DSDL_RUNTIME_ASSERT(buf != NULL);
    const size_t bits = dsdl_runtime_saturate_fragment_bits(buf_size_bytes, off_bits, dsdl_runtime_choose_min(len_bits, 8U));DSDL_RUNTIME_ASSERT(bits <= (sizeof(uint8_t) * 8U));
    uint8_t val = 0;
...
{
    DSDL_RUNTIME_ASSERT(buf != NULL);
    const size_t bits = dsdl_runtime_saturate_fragment_bits(buf_size_bytes, off_bits, dsdl_runtime_choose_min(len_bits, 16U));
...
static inline uint32_t dsdl_runtime_get_u32(const uint8_t* const buf,
                                     ...
                                     const uint8_t len_bits)
{DSDL_RUNTIME_ASSERT(buf != NULL);
    const size_t bits = dsdl_runtime_saturate_fragment_bits(buf_size_bytes, off_bits, dsdl_runtime_choose_min(len_bits, 32U));
...
static inline uint64_t dsdl_runtime_get_u64(const uint8_t* const buf,
                                     ...
                                     const uint8_t len_bits)
{DSDL_RUNTIME_ASSERT(buf != NULL);
    const size_t bits = dsdl_runtime_saturate_fragment_bits(buf_size_bytes, off_bits, dsdl_runtime_choose_min(len_bits, 64U));DSDL_RUNTIME_ASSERT(bits <= (sizeof(uint64_t) * 8U));
...
static inline int8_t dsdl_runtime_set_f16(
    uint8_t* const buf,
    const size_t buf_size_bytes,
    const size_t off_bits,
    const float value)
...
static inline float dsdl_runtime_get_f16(
...
static inline int8_t dsdl_runtime_set_f32(
    uint8_t* const buf,
    const size_t buf_size_bytes,
    const size_t off_bits,
    const float value)
...
static inline float dsdl_runtime_get_f32(
    const uint8_t* const buf,
    const size_t buf_size_bytes,
    const size_t off_bits)
{
...
static inline int8_t dsdl_runtime_set_f64(
    uint8_t* const buf,
    const size_t buf_size_bytes,
    const size_t off_bits,
    const double value)
...
static inline double dsdl_runtime_get_f64(
    const uint8_t* const buf,
    const size_t buf_size_bytes,
    const size_t off_bits)
{
