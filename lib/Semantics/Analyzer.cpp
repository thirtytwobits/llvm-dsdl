...
public:
  AnalyzerImpl(const ASTModule &module, const SemanticOptions &options,
    ...
    for (std::size_t i = 0; i < module_.definitions.size(); ++i) {
      ...
      indexByKey_.emplace(
          typeKey(d.info.fullName, d.info.majorVersion, d.info.minorVersion), i);
    }
  }
...
private:
  ...
  resolveCompositeType(const DiscoveredDefinition &owner,
                       const VersionedTypeExprAST &type,
                       const SourceLocation &location, bool reportError) {
    ...
    if (type.nameComponents.size() == 1) {
      if (!owner.namespaceComponents.empty()) {
        candidates.push_back(joinComponents(owner.namespaceComponents) + "." +
                             type.nameComponents.front());
      } else {
    ...
    if (reportError) {
      diagnostics_.error(location,
                         "unresolved composite type: " + explicitName + "." +
                             std::to_string(type.major) + "." +
  ...
  std::optional<SemanticDefinition> *analyzeOne(std::size_t idx) {
    ...
    if (!sem.isService && markerCount > 0) {
      diagnostics_.error(parsed.ast.location,
                         "service response marker is only valid in service types");
    }
    if (markerCount > 1) {
      diagnostics_.error(parsed.ast.location,
                         "duplicated service response marker");
    }
    ...
    if (sem.isService) {
      ...
      if (response.deprecated) {
        diagnostics_.error(parsed.ast.location,
                           "@deprecated is only allowed in service request section");
      }
  ...
  TypeLayout analyzeType(const DiscoveredDefinition &owner, const TypeExprAST &type,
                         ValueEnv &env,
    ...
    auto baseFromScalar = [&]() -> TypeLayout {
      ...
      if (def.isService) {
        diagnostics_.error(type.location,
                           "service types are not valid field types");
        return out;
      }
      ...
      layout.resolved.compositeType = SemanticTypeRef{
          def.info.fullName,
      ...
      layout.resolved.compositeExtentBits =
          sec.extentBits.value_or(sec.offsetAtEnd.max());
      if (sec.sealed) {
    ...
    if (std::holds_alternative<VoidTypeExprAST>(type.scalar)) {
      diagnostics_.error(type.location,
                         "void type is not allowed as array element type");
      return out;
    }
    ...
    auto capValue =
        evaluateExpression(*type.arrayCapacity, env, diagnostics_, type.location,
                           options_.strict && !options_.compatMode, resolver);
    if (!capValue || !std::holds_alternative<Rational>(capValue->data) ||
      ...
      if (options_.compatMode) {
        diagnostics_.warning(
            type.location,
            "compat mode: array capacity expression fallback to 1");
        capacity = 1;
      } else {
        diagnostics_.error(type.location,
                           "array capacity expression must yield integer rational");
        return out;
      }
    } else {
    ...
    if (type.arrayKind == ArrayKind::Fixed) {
      if (capacity < 1) {
        if (options_.compatMode) {
          diagnostics_.warning(type.location,
                               "compat mode: fixed array capacity clamped to 1");
          capacity = 1;
        } else {
          diagnostics_.error(type.location,
                             "fixed-length array capacity must be positive");
          return out;
        }
      }
      TypeLayout layout;
    ...
    if (type.arrayKind == ArrayKind::VariableExclusive) {
      if (capacity <= 1) {
        if (options_.compatMode) {
          diagnostics_.warning(type.location,
                               "compat mode: exclusive bound clamped to 2");
          capacity = 2;
        } else {
          diagnostics_.error(type.location,
                             "exclusive variable-length array bound must be > 1");
          return out;
        }
      }
      capacity -= 1;
      ...
      if (capacity < 1) {
        if (options_.compatMode) {
          diagnostics_.warning(type.location,
                               "compat mode: inclusive bound clamped to 1");
          capacity = 1;
        } else {
          diagnostics_.error(type.location,
                             "inclusive variable-length array bound must be >= 1");
          return out;
        }
      }
    }
  ...
  bool checkConstantCompatibility(const ConstantDeclAST &decl, const Value &value) {
    ...
    if (!prim) {
      diagnostics_.error(decl.location,
                         "constant attributes must be primitive-typed");
      return false;
    }
    ...
    if (prim->kind == PrimitiveKind::Bool) {
      if (!std::holds_alternative<bool>(value.data)) {
        diagnostics_.error(decl.location,
                           "boolean constants must be initialized from bool");
        return false;
      }
      return true;
    }

    if (prim->kind == PrimitiveKind::UnsignedInt ||
        prim->kind == PrimitiveKind::SignedInt) {
      if (auto r = std::get_if<Rational>(&value.data)) {
        if (!r->isInteger()) {
          diagnostics_.error(decl.location,
                             "integer constants require integer rational values");
          return false;
        }
        return true;
      }
      if (auto s = std::get_if<std::string>(&value.data)) {
        if (prim->kind == PrimitiveKind::UnsignedInt && prim->bitLength == 8 &&
            s->size() == 1 && static_cast<unsigned char>((*s)[0]) <= 127) {
          return true;
        }
      }
      diagnostics_.error(decl.location,
                         "integer constants require rational or ASCII single-char string");
      return false;
    }
    ...
    if (prim->kind == PrimitiveKind::Float) {
      if (!std::holds_alternative<Rational>(value.data)) {
        diagnostics_.error(decl.location,
                           "floating constants require rational values");
        return false;
      }
      return true;
    }
    ...
    diagnostics_.error(decl.location,
                       "byte/utf8 are not valid constant attribute types");
    return false;
  }
  ...
  SemanticSection analyzeSection(const DiscoveredDefinition &owner,
    ...
    TypeAttributeResolver typeAttrResolver =
        [&](const TypeExprAST &typeExpr, const std::string &attributeName,
            const SourceLocation &location) -> std::optional<Value> {
      const auto unsupported = [&](const std::string &message) -> std::optional<Value> {
        if (options_.compatMode) {
          diagnostics_.warning(location, "compat mode: " + message +
                                             " evaluated as 0");
      ...
      if (typeExpr.arrayKind != ArrayKind::None) {
        return unsupported("unsupported metaserializable attribute: " +
                           attributeName);
      }
      ...
      auto resolved =
          resolveCompositeType(owner, *versioned, location, !options_.compatMode);
      if (!resolved) {
        if (options_.compatMode) {
          diagnostics_.warning(
              location, "compat mode: unresolved composite type in attribute "
      ...
      if (!resolvedDef || !*resolvedDef) {
        ...
        diagnostics_.error(location,
                           "failed to analyze dependent type: " + resolved->second);
        return std::nullopt;
      }
      const auto &def = **resolvedDef;
      if (def.isService) {
        return unsupported("service types do not expose value attributes: " +
                           resolved->second);
      }
    ...
    auto updateOffsetEnv = [&]() {
      if (section.isUnion) {
        env["_offset_"] = Value{bitLengthSetToValueSet(
            computeUnionOffsetFromSeenFields())};
      } else {
    ...
    for (const auto &stmt : statements) {
      ...
      if (std::holds_alternative<DirectiveAST>(stmt)) {
        ...
        if (d.kind == DirectiveKind::Unknown) {
          if (options_.compatMode) {
            diagnostics_.warning(d.location,
                                 "compat mode: unknown directive @" + d.rawName);
          } else {
            diagnostics_.error(d.location,
                               "unknown directive @" + d.rawName);
          }
          continue;
        }
        ...
        if (d.kind == DirectiveKind::Union) {
          if (d.expression) {
            diagnostics_.error(d.location,
                               "@union does not accept an expression");
          }
          if (section.isUnion) {
          ...
          if (attributesStarted) {
            diagnostics_.error(d.location,
                               "@union must appear before first attribute");
          }
        ...
        if (d.kind == DirectiveKind::Deprecated) {
          if (d.expression) {
            diagnostics_.error(d.location,
                               "@deprecated does not accept an expression");
          }
          if (isResponse) {
            diagnostics_.error(d.location,
                               "@deprecated is only valid in service request section");
          }
          if (section.deprecated) {
            diagnostics_.error(d.location,
                               "duplicated @deprecated directive");
          }
          if (attributesStarted) {
            diagnostics_.error(d.location,
                               "@deprecated must be placed before first attribute");
          }
        ...
        if (d.kind == DirectiveKind::Extent) {
          if (serializationModeSet) {
            diagnostics_.error(d.location,
                               "serialization mode already set before @extent");
            continue;
          }
          if (!d.expression) {
            diagnostics_.error(d.location,
                               "@extent requires an expression");
            continue;
          }
          if (exprContainsOffset(d.expression) && section.isUnion) {
            unionOffsetUsed = true;
          }
          auto v = evaluateExpression(*d.expression, env, diagnostics_, d.location,
                                      options_.strict && !options_.compatMode,
                                      &typeAttrResolver);
          if (!v || !std::holds_alternative<Rational>(v->data) ||
              !std::get<Rational>(v->data).isInteger()) {
            diagnostics_.error(d.location,
        ...
        if (d.kind == DirectiveKind::Sealed) {
          if (serializationModeSet) {
            diagnostics_.error(d.location,
                               "serialization mode already set before @sealed");
            continue;
          }
          if (d.expression) {
            diagnostics_.error(d.location,
                               "@sealed does not accept an expression");
          }
          section.sealed = true;
          serializationModeSet = true;
          continue;
        ...
        if (d.kind == DirectiveKind::Assert || d.kind == DirectiveKind::Print) {
          ...
          if (!d.expression && d.kind == DirectiveKind::Assert) {
            if (options_.compatMode) {
              diagnostics_.warning(d.location,
                                   "compat mode: @assert without expression ignored");
            } else {
          ...
          if (d.expression) {
            auto value = evaluateExpression(*d.expression, env, diagnostics_, d.location,
                                            options_.strict && !options_.compatMode,
                                            &typeAttrResolver);
            if (d.kind == DirectiveKind::Assert) {
              if (!value || !std::holds_alternative<bool>(value->data)) {
                if (options_.compatMode) {
                  diagnostics_.warning(d.location,
                                       "compat mode: non-bool @assert ignored");
                } else {
                  diagnostics_.error(d.location,
                ...
                if (options_.compatMode) {
                  diagnostics_.warning(d.location,
                                       "compat mode: assertion failed but ignored");
                } else {
                  diagnostics_.error(d.location,
                                     "assertion failed");
                }
      ...
      if (std::holds_alternative<ConstantDeclAST>(stmt)) {
        ...
        if (serializationModeSet) {
          diagnostics_.error(c.location,
                             "attributes cannot follow @extent/@sealed");
        }
        if (names.contains(c.name)) {
          diagnostics_.error(c.location,
                             "duplicate attribute name: " + c.name);
          continue;
        }

        auto value = evaluateExpression(*c.value, env, diagnostics_, c.location,
                                        options_.strict && !options_.compatMode,
                                        &typeAttrResolver);
        if (!value) {
          continue;
      ...
      if (std::holds_alternative<FieldDeclAST>(stmt)) {
        ...
        attributesStarted = true;
        if (serializationModeSet) {
          diagnostics_.error(f.location,
                             "attributes cannot follow @extent/@sealed");
        }

        if (!f.isPadding && names.contains(f.name)) {
          diagnostics_.error(f.location,
                             "duplicate attribute name: " + f.name);
          continue;
        }
        if (!f.isPadding) {
          names.insert(f.name);
        ...
        if (section.isUnion && f.isPadding) {
          diagnostics_.error(f.location,
                             "tagged unions cannot contain padding fields");
          continue;
        }
        if (section.isUnion && unionOffsetUsed) {
          diagnostics_.error(
              f.location,
              "field after _offset_ usage in union is not allowed by spec");
        }
    ...
    if (section.isUnion) {
      if (unionFields.size() < 2) {
        diagnostics_.error(owner.filePath.empty()
                               ? SourceLocation{"<unknown>", 1, 1}
      ...
      const auto b = ceilLog2(static_cast<std::int64_t>(unionFields.size()));
      const auto unionTagBits = static_cast<std::uint32_t>(
          pow2ceil(std::max<std::int64_t>(8, b)));
      ...
      for (const auto *d : defs) {
        ...
        if (d->info.fixedPortId) {
          if (lastFixed && *lastFixed != *d->info.fixedPortId) {
            diagnostics_.error({d->info.filePath, 1, 1},
                               "fixed port-ID mismatch under same major version");
          }
    ...
    for (std::size_t i = 0; i < results_.size(); ++i) {
      ...
      for (std::size_t j = i + 1; j < results_.size(); ++j) {
        ...
        if (*results_[i]->info.fixedPortId == *results_[j]->info.fixedPortId &&
            results_[i]->info.majorVersion > 0 &&
            results_[j]->info.majorVersion > 0) {
          diagnostics_.error(
              {results_[j]->info.filePath, 1, 1},
