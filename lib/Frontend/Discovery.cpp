...
void discoverInRoot(const std::filesystem::path &root, bool isPrimaryRoot,
  ...
  if (!std::filesystem::exists(canonicalRoot)) {
    diagnostics.error({root.string(), 1, 1},
                      "namespace root does not exist: " + root.string());
    return;
  }

  static const std::regex fileRe(
      R"(^((\d+)\.)?([A-Za-z_][A-Za-z0-9_]*)\.(\d+)\.(\d+)\.dsdl$)");
  const std::string rootNamespace = canonicalRoot.filename().string();
  if (!isValidNameComponent(rootNamespace)) {
    diagnostics.error({root.string(), 1, 1},
                      "invalid root namespace directory name: " +
                          rootNamespace);
    return;
  }

  for (const auto &entry :
       std::filesystem::recursive_directory_iterator(canonicalRoot)) {
    ...
    if (!std::regex_match(fileName, m, fileRe)) {
      diagnostics.error({path.string(), 1, 1},
                        "invalid DSDL filename format: " + fileName);
      continue;
    }
    ...
    if (def.majorVersion == 0 && def.minorVersion == 0) {
      diagnostics.error({path.string(), 1, 1},
                        "version 0.0 is not allowed in DSDL definitions");
      continue;
    }

    const auto relativeParent =
        std::filesystem::relative(path.parent_path(), canonicalRoot);
    ...
    for (const std::string &comp : ns) {
      if (!isValidNameComponent(comp)) {
        diagnostics.error({path.string(), 1, 1},
                          "invalid namespace component: " + comp);
    ...
    if (!readTextFile(path, def.text)) {
      diagnostics.error({path.string(), 1, 1},
                        "failed to read DSDL source file");
      continue;
    }
...
discoverDefinitions(const std::vector<std::string> &rootNamespaceDirs,
  ...
  std::sort(definitions.begin(), definitions.end(),
            [](const DiscoveredDefinition &a, const DiscoveredDefinition &b) {
  ...
  for (const auto &def : definitions) {
    ...
    const auto [itName, insertedName] =
        caseInsensitiveNames.emplace(lowerName, def.fullName);
    if (!insertedName && itName->second != def.fullName) {
      diagnostics.error(
          {def.filePath, 1, 1},
          "name collision on case-insensitive filesystem: " + def.fullName +
              " conflicts with " + itName->second);
    }

    const std::string versionKey = lowerName + ":" +
                                   std::to_string(def.majorVersion) + ":" +
    ...
    if (!insertedV) {
      diagnostics.error({def.filePath, 1, 1},
                        "duplicate definition version: " + def.fullName + "." +
                            std::to_string(def.majorVersion) + "." +
                            std::to_string(def.minorVersion));
    }
  }
