...
public:
                               ...
                               const LoweredSectionFacts* const sectionFacts)
    {
        ...
        for (const auto& step : renderIR.steps)
        {
            ...
            case LoweredRenderStepKind::UnionDispatch:
                emitSerializeUnion(out, section, step.unionBranches, "obj", 1, sectionFacts, renderIR.helperBindings);
                break;
                                 ...
                                 const LoweredSectionFacts* const sectionFacts)
    {
        ...
        emitLine(out,
                 1,
                 "*inout_buffer_size_bytes = static_cast<std::size_t>(dsdl_runtime_choose_min(offset_bits, "
                 "capacity_bits) / 8U);");
        emitLine(out, 1, "return static_cast<std::int8_t>(DSDL_RUNTIME_SUCCESS);");
        emitLine(out, 0, "}");
        out << "\n";
