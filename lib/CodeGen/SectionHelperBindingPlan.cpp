...
buildSectionHelperBindingPlan(const SemanticSection &section,
  ...
  if (section.isUnion) {
    if (shared.unionTagValidate &&
        emittedSymbols.insert(shared.unionTagValidate->symbol).second) {
      out.unionTagValidate = shared.unionTagValidate;
    }
    const auto symbol = (direction == HelperBindingDirection::Serialize)
                            ? (sectionFacts ? sectionFacts->serUnionTagHelper
                                            : std::string{})
                            : (sectionFacts ? sectionFacts->deserUnionTagHelper
                                            : std::string{});
    if (!symbol.empty() && emittedSymbols.insert(symbol).second) {
      out.unionTagMask = UnionTagMaskBindingDescriptor{
          symbol, resolveUnionTagBits(section, sectionFacts)};
    }
  }
  ...
  for (const auto &field : section.fields) {
    ...
    if (delimiterDescriptor &&
        emittedSymbols.insert(delimiterDescriptor->symbol).second) {
      out.delimiterValidateBindings.push_back(
          DelimiterValidateBindingDescriptor{delimiterDescriptor->symbol});
    }

    const auto arrayDescriptor = buildArrayLengthHelperDescriptor(
        field, loweredFieldArrayPrefixBits(sectionFacts, field.name),
        (direction == HelperBindingDirection::Serialize)
            ? (fieldFacts ? fieldFacts->serArrayLengthPrefixHelper
                          : std::string{})
    ...
    if (!arrayDescriptor->prefixSymbol.empty() &&
        emittedSymbols.insert(arrayDescriptor->prefixSymbol).second) {
      out.arrayPrefixBindings.push_back(ArrayPrefixBindingDescriptor{
          arrayDescriptor->prefixSymbol, arrayDescriptor->prefixBits});
    }
    if (!arrayDescriptor->validateSymbol.empty() &&
        emittedSymbols.insert(arrayDescriptor->validateSymbol).second) {
      out.arrayValidateBindings.push_back(ArrayValidateBindingDescriptor{
          arrayDescriptor->validateSymbol, arrayDescriptor->capacity});
    }
  }
