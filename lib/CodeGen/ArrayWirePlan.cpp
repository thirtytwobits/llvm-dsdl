...
ArrayWirePlan buildArrayWirePlan(
  ...
  out.prefixBits =
      prefixBitsOverride.value_or(static_cast<std::uint32_t>(type.arrayLengthPrefixBits));
  out.descriptor = resolveArrayLengthHelperDescriptor(type, fieldFacts,
                                                      out.prefixBits, direction);
  return out;
}
