...
SharedSerDesHelperDescriptors buildSharedSerDesHelperDescriptors(
  ...
  if (!capacityCheckSymbol.empty()) {
    ...
    descriptor.requiredBits = std::max<std::int64_t>(
        0, section.serializationBufferSizeBits);
...
std::optional<ArrayLengthHelperDescriptor> buildArrayLengthHelperDescriptor(
  ...
  const bool variableArray = (kind == ArrayKind::VariableInclusive) ||
  ...
  out.prefixBits = prefixBitsOverride.value_or(
      static_cast<std::uint32_t>(std::max<std::int64_t>(
          0, type.arrayLengthPrefixBits)));
...
std::optional<ArrayLengthHelperDescriptor> buildArrayLengthHelperDescriptor(
  ...
  return buildArrayLengthHelperDescriptor(field.resolvedType, prefixBitsOverride,
                                          prefixSymbol, validateSymbol);
}
...
std::optional<ScalarHelperDescriptor> buildScalarHelperDescriptor(
  ...
  out.bitLength = static_cast<std::uint32_t>(std::max<std::int64_t>(
      0, type.bitLength));
