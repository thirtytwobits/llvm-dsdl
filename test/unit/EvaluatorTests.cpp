...
std::shared_ptr<llvmdsdl::ExprAST>
parseAssertExpression(const std::string &expression,
...
bool hasErrorContaining(const llvmdsdl::DiagnosticEngine &diag,
...
bool runEvaluatorTests() {
  {
    ...
    auto value = llvmdsdl::evaluateExpression(*expr, env, diag, expr->location,
                                              /*strictMode=*/true);
    if (value) {
...

  {
    ...
    llvmdsdl::TypeAttributeResolver resolver =
        [&](const llvmdsdl::TypeExprAST &type, const std::string &attribute,
            const llvmdsdl::SourceLocation &) -> std::optional<llvmdsdl::Value> {
      resolverCalled = true;
      const auto *versioned =
          std::get_if<llvmdsdl::VersionedTypeExprAST>(&type.scalar);
...

  {
    ...
    llvmdsdl::TypeAttributeResolver resolver =
        [&](const llvmdsdl::TypeExprAST &, const std::string &attribute,
            const llvmdsdl::SourceLocation &) -> std::optional<llvmdsdl::Value> {
