...
bool runLoweredBodyPlanTests() {
  {
    ...
    scalar.name = "scalar";
    scalar.resolvedType.scalarCategory =
    ...
    const auto serPlan = llvmdsdl::buildLoweredBodyPlan(
        section, &facts, llvmdsdl::HelperBindingDirection::Serialize);
    const auto desPlan = llvmdsdl::buildLoweredBodyPlan(
        section, &facts, llvmdsdl::HelperBindingDirection::Deserialize);

    if (!serPlan.helperBindings.capacityCheck ||
        serPlan.helperBindings.capacityCheck->symbol != "capacity_check" ||
        serPlan.helperBindings.capacityCheck->requiredBits != 80) {
      std::cerr << "serialize lowered body capacity helper mismatch\n";
      return false;
    }
    if (serPlan.statements.orderedFields.size() != 2 ||
        serPlan.statements.orderedFields[0].field == nullptr ||
    ...
    if (serPlan.helperBindings.arrayPrefixBindings.size() != 1 ||
        serPlan.helperBindings.arrayPrefixBindings[0].symbol !=
            "payload_prefix_ser" ||
    ...
    if (desPlan.helperBindings.arrayPrefixBindings.size() != 1 ||
        desPlan.helperBindings.arrayPrefixBindings[0].symbol !=
            "payload_prefix_des" ||
...

  {
    ...
    alpha.resolvedType.scalarCategory =
        llvmdsdl::SemanticScalarCategory::UnsignedInt;
    ...
    beta.unionTagBits = 8;
    beta.resolvedType.scalarCategory =
        llvmdsdl::SemanticScalarCategory::UnsignedInt;
    ...
    if (serPlan.statements.unionBranches.size() != 2 ||
        serPlan.statements.unionBranches[0].field == nullptr ||
    ...
    if (!serPlan.helperBindings.unionTagMask ||
        serPlan.helperBindings.unionTagMask->symbol != "union_tag_ser" ||
        serPlan.helperBindings.unionTagMask->bits != 8) {
      std::cerr << "serialize lowered body union tag mask mismatch\n";
      return false;
    }
    if (!desPlan.helperBindings.unionTagMask ||
        desPlan.helperBindings.unionTagMask->symbol != "union_tag_des" ||
