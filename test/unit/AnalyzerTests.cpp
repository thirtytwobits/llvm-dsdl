...
bool runAnalyzerTests() {
  ...
  llvmdsdl::Parser parser("uavcan.test.UnionOffset.1.0.dsdl", std::move(tokens),
                          parseDiag);
  ...
  if (section.fields[0].unionOptionIndex != 0 ||
      section.fields[1].unionOptionIndex != 1 ||
      section.fields[0].unionTagBits != 8 ||
      section.fields[1].unionTagBits != 8) {
    std::cerr << "unexpected union tag metadata\n";
    return false;
  }
  if (section.minBitLength != 16 || section.maxBitLength != 24 ||
      section.serializationBufferSizeBits != 24 || section.fixedSize) {
