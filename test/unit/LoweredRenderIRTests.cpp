...
bool runLoweredRenderIRTests() {
  {
    ...
    const auto renderIR = llvmdsdl::buildLoweredBodyRenderIR(
        section, &facts, llvmdsdl::HelperBindingDirection::Serialize);

    if (!renderIR.helperBindings.capacityCheck ||
        renderIR.helperBindings.capacityCheck->symbol != "cap") {
    ...
    if (renderIR.steps[0].kind != llvmdsdl::LoweredRenderStepKind::Field ||
        renderIR.steps[0].fieldStep.field == nullptr ||
        renderIR.steps[0].fieldStep.field->name != "scalar") {
      std::cerr << "lowered render ir field step mismatch\n";
      return false;
    }
    if (renderIR.steps[1].kind != llvmdsdl::LoweredRenderStepKind::Padding ||
        renderIR.steps[1].fieldStep.field == nullptr ||
        !renderIR.steps[1].fieldStep.field->isPadding) {
...

  {
    ...
    if (renderIR.steps.size() != 1 ||
        renderIR.steps.front().kind !=
            llvmdsdl::LoweredRenderStepKind::UnionDispatch) {
    ...
    if (!renderIR.helperBindings.unionTagMask ||
        renderIR.helperBindings.unionTagMask->symbol != "tag_des") {
