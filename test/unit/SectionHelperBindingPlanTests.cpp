...
bool runSectionHelperBindingPlanTests() {
  {
    ...
    scalarField.name = "x";
    scalarField.resolvedType.scalarCategory =
        llvmdsdl::SemanticScalarCategory::SignedInt;
    ...
    arrayField.name = "arr";
    arrayField.resolvedType.scalarCategory =
        llvmdsdl::SemanticScalarCategory::UnsignedInt;
    ...
    compositeField.resolvedType.scalarCategory =
        llvmdsdl::SemanticScalarCategory::Composite;
    ...
    const auto serPlan = llvmdsdl::buildSectionHelperBindingPlan(
        section, &facts, llvmdsdl::HelperBindingDirection::Serialize);
    const auto desPlan = llvmdsdl::buildSectionHelperBindingPlan(
        section, &facts, llvmdsdl::HelperBindingDirection::Deserialize);
    ...
    if (serPlan.scalarBindings.size() != 1 ||
        serPlan.scalarBindings.front().symbol != "signed_ser") {
      std::cerr << "serialize plan scalar helper mismatch\n";
      return false;
    }
    if (desPlan.scalarBindings.size() != 1 ||
        desPlan.scalarBindings.front().symbol != "signed_des") {
      std::cerr << "deserialize plan scalar helper mismatch\n";
      return false;
    }
    if (serPlan.arrayPrefixBindings.size() != 1 ||
        serPlan.arrayPrefixBindings.front().symbol != "arr_prefix_ser" ||
        serPlan.arrayPrefixBindings.front().bits != 8) {
      std::cerr << "serialize plan array prefix helper mismatch\n";
      return false;
    }
    if (desPlan.arrayPrefixBindings.size() != 1 ||
        desPlan.arrayPrefixBindings.front().symbol != "arr_prefix_des" ||
    ...
    if (serPlan.delimiterValidateBindings.size() != 1 ||
        serPlan.delimiterValidateBindings.front().symbol !=
            "delimiter_validate") {
...

  {
    ...
    if (!serPlan.unionTagValidate ||
        serPlan.unionTagValidate->symbol != "union_validate" ||
        serPlan.unionTagValidate->allowedTags.size() != 2 ||
        serPlan.unionTagValidate->allowedTags[0] != 0 ||
        serPlan.unionTagValidate->allowedTags[1] != 2) {
      std::cerr << "union tag validate helper mismatch\n";
      return false;
    }
    if (!serPlan.unionTagMask ||
        serPlan.unionTagMask->symbol != "union_tag_ser" ||
        serPlan.unionTagMask->bits != 8) {
      std::cerr << "serialize union tag mask helper mismatch\n";
      return false;
    }
    if (!desPlan.unionTagMask ||
        desPlan.unionTagMask->symbol != "union_tag_des" ||
        desPlan.unionTagMask->bits != 8) {
