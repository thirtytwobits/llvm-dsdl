...
bool runParserTests() {
  ...
  const std::string typeLiteralText =
  ...
  llvmdsdl::Parser typeLiteralParser("type_literal_expr.dsdl",
                                     std::move(typeLiteralTokens),
                                     typeLiteralDiag);
  ...
  if (typeLiteralDef->statements.size() != 2) {
    std::cerr << "unexpected type-literal statement count: "
              << typeLiteralDef->statements.size() << "\n";
    return false;
  }

  const auto *field =
      std::get_if<llvmdsdl::FieldDeclAST>(&typeLiteralDef->statements[0]);
  if (!field) {
  ...
  const auto *capacityBinary = std::get_if<llvmdsdl::ExprAST::Binary>(
      &field->type.arrayCapacity->value);
  ...
  const auto *lhsType =
      std::get_if<llvmdsdl::ExprAST::TypeLiteral>(&capacityBinary->lhs->value);
  if (!lhsType) {
    std::cerr << "expected type literal on attribute LHS\n";
    return false;
  }
  const auto *rhsId = std::get_if<llvmdsdl::ExprAST::Identifier>(
      &capacityBinary->rhs->value);
  ...
  const auto *versioned =
      std::get_if<llvmdsdl::VersionedTypeExprAST>(&lhsType->type.scalar);
  if (!versioned) {
    std::cerr << "expected versioned type literal\n";
    return false;
  }
  if (versioned->nameComponents !=
          std::vector<std::string>{"uavcan", "file", "Path"} ||
      versioned->major != 2 || versioned->minor != 0) {
