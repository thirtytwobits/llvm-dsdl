...
bool hasSubstring(const std::vector<std::string> &lines,
...
bool runHelperBindingRenderTests() {
  {
    const auto lines = llvmdsdl::renderUnionTagValidateBinding(
        llvmdsdl::HelperBindingRenderLanguage::Cpp, "union_check",
        {0, 2});
  ...
  {
    const auto lines = llvmdsdl::renderDelimiterValidateBinding(
        llvmdsdl::HelperBindingRenderLanguage::Rust, "delim_check");
    if (!hasSubstring(
            lines,
...

  {
    ...
    const auto lines = llvmdsdl::renderScalarBinding(
        llvmdsdl::HelperBindingRenderLanguage::Cpp,
        llvmdsdl::ScalarBindingRenderDirection::Serialize, "sat_u3",
        descriptor);
...

  {
    ...
    const auto lines = llvmdsdl::renderScalarBinding(
        ...
        llvmdsdl::ScalarBindingRenderDirection::Deserialize, "des_i3",
        descriptor);
    if (!hasSubstring(lines, "(raw & 4u64) != 0u64") ||
        !hasSubstring(lines, "(raw | (!7u64)) as i64")) {
  ...
  {
    const auto lines = llvmdsdl::renderArrayPrefixBinding(
        llvmdsdl::HelperBindingRenderLanguage::Cpp, "arr_prefix", 8);
...

  {
    ...
    const auto lines = llvmdsdl::renderSectionHelperBindings(
        plan, llvmdsdl::HelperBindingRenderLanguage::Rust,
