...
bool runSerDesStatementPlanTests() {
  {
    ...
    header.name = "header";
    header.resolvedType.scalarCategory =
    ...
    payload.resolvedType.scalarCategory =
        llvmdsdl::SemanticScalarCategory::UnsignedInt;
    ...
    if (plan.orderedFields[0].field == nullptr ||
        plan.orderedFields[0].field->name != "header") {
      std::cerr << "ordered field[0] mismatch\n";
      return false;
    }
    if (plan.orderedFields[1].field == nullptr ||
        !plan.orderedFields[1].field->isPadding) {
      std::cerr << "ordered field[1] should be padding\n";
      return false;
    }
    if (plan.orderedFields[2].field == nullptr ||
        plan.orderedFields[2].field->name != "payload" ||
        !plan.orderedFields[2].arrayLengthPrefixBits ||
        *plan.orderedFields[2].arrayLengthPrefixBits != 16) {
      std::cerr << "ordered field[2] array metadata mismatch\n";
      return false;
    }
    if (plan.orderedFields[2].fieldFacts == nullptr ||
        plan.orderedFields[2].fieldFacts->serArrayLengthPrefixHelper !=
            "payload_prefix_ser") {
      std::cerr << "ordered field[2] lowered facts mismatch\n";
      return false;
    }
    if (plan.unionBranches[0].field == nullptr ||
        plan.unionBranches[0].field->name != "header" ||
        plan.unionBranches[1].field == nullptr ||
        plan.unionBranches[1].field->name != "payload") {
...

  {
    ...
    option.resolvedType.scalarCategory =
    ...
    if (plan.orderedFields.front().arrayLengthPrefixBits ||
        plan.orderedFields.front().fieldFacts != nullptr) {
...

  {
    ...
    first.name = "first";
    first.resolvedType.scalarCategory =
        llvmdsdl::SemanticScalarCategory::UnsignedInt;
    ...
    second.name = "second";
    second.resolvedType.scalarCategory =
        llvmdsdl::SemanticScalarCategory::UnsignedInt;
    ...
    if (plan.orderedFields[0].field == nullptr ||
        plan.orderedFields[0].field->name != "second" ||
        plan.orderedFields[1].field == nullptr ||
        plan.orderedFields[1].field->name != "first") {
      std::cerr << "step-index ordering mismatch\n";
      return false;
    }
    if (plan.unionBranches[0].field == nullptr ||
        plan.unionBranches[0].field->name != "second" ||
        plan.unionBranches[1].field == nullptr ||
        plan.unionBranches[1].field->name != "first") {
